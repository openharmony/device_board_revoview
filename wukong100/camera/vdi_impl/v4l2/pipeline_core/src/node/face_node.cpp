...
FaceNode::FaceNode(const std::string &name, const std::string &type, const std::string &cameraId) : NodeBase(name, type, cameraId), metaDataSize_(0)
{
...
void FaceNode::DeliverBuffer(std::shared_ptr<IBuffer>& buffer)
{
    ...
    for (auto& it : outPutPorts_) {
        if (it->format_.streamId_ == id) {
            ...
            CAMERA_LOGI("FaceNode deliver buffer streamid = %{public}d", it->format_.streamId_);
            return;
        }
    }
}
...
RetCode FaceNode::CopyBuffer(unsigned char *sourceBuffer, std::shared_ptr<IBuffer>& outPutBuffer, int32_t dataSize)
{
...
{
    const int entryCapacity = 30;
    const int dataCapacity = 2000;
    std::unique_lock <std::mutex> lock(mLock_);
