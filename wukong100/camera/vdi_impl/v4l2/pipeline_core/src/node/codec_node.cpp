...
AvcH264EncInterface* CodecNode::CreateAvcH264EncInterface() {
  ...
  if (createInstance == nullptr) {
    ...
    if (dlsym_error) {
      CAMERA_LOGE(
          "Cannot load symbol CreateAvcH264Encoder, %{public}s, %{public}p",
...
unsigned char CodecNode::Clip(int value) {
  ...
  return static_cast<unsigned char>(value > BYTEMAXVAL ? BYTEMAXVAL
                                                       : value < 0 ? 0 : value);
}
...
int CodecNode::Yuv420ToH264WithUnisoc(std::shared_ptr<IBuffer>& buffer,
  ...
  if (cameraId == CAMERA_FIRST) {
    ...
    Yuv420_Rot_VMirror(bufferRotate_, (u_char*)buffer->GetVirAddress(), width,
                       height);
  ...
  if (h264Encoder != nullptr) {
    ...
    CAMERA_LOGI(
        "Yuv420ToH264WithUnisoc:: h264Encoder status is %{public}d, codecnode "
        "addr is %{public}p",
        status.load(), this);
    if (status == 0) {
      status = 1;
    ...
    int type = 0;
    int ret1 = h264Encoder->vsp_enc(&input, (char*)(buffer->GetVirAddress()),
                                    frameSize, type);
    if (ret1 == 0) {
      if (type == 0) {
...
void CodecNode::DeliverBuffer(std::shared_ptr<IBuffer>& buffer) {
  ...
  CAMERA_LOGD(
      "CodecNode::DeliverBuffer StreamId %{public}d, encode type : %{public}d",
  ...
  if (buffer->GetEncodeType() == VDI::Camera::V1_0::ENCODE_TYPE_JPEG) {
    ...
    const int YUV420_SIZE_UP = 3;
    const int YUV420_SIZE_DOWN = 2;
    int yuv_length = buffer->GetWidth() * buffer->GetHeight() * YUV420_SIZE_UP /
                     YUV420_SIZE_DOWN;
