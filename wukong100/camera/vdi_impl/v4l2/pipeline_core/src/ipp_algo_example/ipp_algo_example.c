...
int Process(IppAlgoBuffer *inBuffer[], int inBufferCount,
  ...
  for (int i = 0; i < inBufferCount; i++) {
    if (inBuffer[i] != NULL) {
      printf(
          "in buffer addr = %p, width = %u, height = %u, stride = %u, size = "
          "%u, id = %d\n",
  ...
  if (inBufferCount == 1) {
    if (inBuffer[0] == NULL || outBuffer == NULL || inBuffer[0]->addr == NULL ||
        outBuffer->addr == NULL) {
      return 0;
    }
  ...
  if (inBufferCount == INBUFFERDOUBLE) {
    ...
    char *startBuffer1 =
        (char *)(inBuffer[0]->addr) + inBuffer[0]->stride * inBuffer[0]->height;
    char *startBuffer2 =
        (char *)(inBuffer[1]->addr) + inBuffer[1]->stride * inBuffer[1]->height;
    if (memcpy_s(startBuffer1,
                 inBuffer[0]->size - inBuffer[0]->stride * inBuffer[0]->height,
                 startBuffer2,
                 inBuffer[0]->stride * inBuffer[0]->height) != 0) {
      printf("memcpy_s failed.");
    }
    return 0;
  }
