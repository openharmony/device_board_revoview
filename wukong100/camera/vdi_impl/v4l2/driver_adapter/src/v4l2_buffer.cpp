...
RetCode HosV4L2Buffers::V4L2QueueBuffer(
  ...
  if (itr != queueBuffers_.end()) {
    ...
    if (rc < 0) {
      bufferLock_.unlock();
      CAMERA_LOGE(
          "HosV4L2Dev::v4l2Handle_->ioctl VIDIOC_QBUF failed: %{public}s\n",
          strerror(errno));
      return RC_ERROR;
    }
...
RetCode HosV4L2Buffers::V4L2DequeueBuffer(int fd) {
  ...
  CAMERA_LOGD(
      "V4L2DequeueBuffer memoryType_  buf.index == %{public}d rc=%{public}d\n",
      buf.index, rc);
  if (rc < 0) {
    CAMERA_LOGE(
        "HosV4L2Dev::v4l2Handle_->ioctl VIDIOC_DQBUF failed: %{public}s\n",
        strerror(errno));
    return RC_ERROR;
  }
  if (memoryType_ == V4L2_MEMORY_MMAP) {
    if (adapterBufferMap_[buf.index].userBufPtr &&
        adapterBufferMap_[buf.index].start) {
      if (memcpy_s(adapterBufferMap_[buf.index].userBufPtr,
                   adapterBufferMap_[buf.index].length,
                   adapterBufferMap_[buf.index].start,
                   adapterBufferMap_[buf.index].length) != 0) {
        rc = RC_ERROR;
      }
    } else {
      rc = RC_ERROR;
  ...
  if (Iter == bufferMap.end()) {
    bufferLock_.unlock();
    CAMERA_LOGE(
        "V4L2DequeueBuffer buf.index == %{public}d is not find in FrameMap\n",
        buf.index);
    return RC_ERROR;
  }
...
RetCode HosV4L2Buffers::V4L2AllocBuffer(int fd, int buffcont) {
  for (int i = 0; i < buffcont; i++) {
    ...
    if (HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_QUERYBUF, &buf) < 0) {
      CAMERA_LOGE(
          "error: HosV4L2Dev::v4l2Handle_->ioctl VIDIOC_QUERYBUF buf.index = "
  ...
  switch (memoryType_) {
    case V4L2_MEMORY_MMAP:
      ...
      if (adapterBufferMap_[buf.index].start == nullptr) {
        adapterBufferMap_[buf.index].start = HosV4L2Dev::v4l2Handle_->mmap(
            NULL, adapterBufferMap_[buf.index].length, PROT_READ | PROT_WRITE,
            bufferType_, fd, adapterBufferMap_[buf.index].offset);
...
RetCode HosV4L2Buffers::V4L2ReleaseBuffers(int fd) {
  ...
  for (auto& mem : adapterBufferMap_) {
    if (mem.second.start) {
      if (HosV4L2Dev::v4l2Handle_->munmap(mem.second.start, mem.second.length) <
          0) {
      }
    }
  }
