...
void HosV4L2UVC::V4L2UvcSearchCapability(const std::string devName,
  ...
  if (inOut) {
    ...
    if (fd < 0) {
      CAMERA_LOGE(
          "UVC:V4L2UvcSearchCapability open %{public}s name %{public}s error\n",
...
void HosV4L2UVC::loopUvcDevice() {
  ...
  while (g_uvcDetectEnable) {
    rc = select(((uDevFd > eventFd) ? uDevFd : eventFd) + 1, &fds, &fds, NULL,
                NULL);
...
void HosV4L2UVC::UpdateV4L2UvcMatchDev(std::string& action,
  ...
  if (subsystem == "video4linux") {
    ...
    if (action == "remove") {
      ...
      if (rc == RC_ERROR) {
        CAMERA_LOGE(
            "UVC:lop V4L2UvcGetCap err rc %{public}d devnode = %{public}s\n",
            rc, devnode.c_str());
        return;
      }
      CAMERA_LOGD("UVC:loop HosV4L2Dev::deviceMatch %{public}s\n",
