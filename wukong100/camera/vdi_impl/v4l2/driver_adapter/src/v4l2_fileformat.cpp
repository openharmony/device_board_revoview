...
RetCode HosFileFormat::V4L2SearchFormat(int fd,
  ...
  for (i = 0; i < fmtMax; ++i) {
    ...
    for (j = 0; j < fmtMax; ++j) {
      ...
      if (HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frmSize) <
          0) {
        break;
      }
      ...
      for (k = 0; k < fmtMax; ++k) {
        ...
        if (HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_ENUM_FRAMEINTERVALS,
                                           &fraMival) < 0) {
          break;
        }
...
RetCode HosFileFormat::V4L2GetCapability(int fd, const std::string& devName,
  ...
  if (SPRDDRIVERNAME != std::string((char*)cap.driver)) {
    return RC_ERROR;
  }
  ...
  CAMERA_LOGD("v4l2 device cameraId = %{public}s\n", cameraId.c_str());
...
int HosFileFormat::V4L2OpenDevice(const std::string& deviceName) {
  ...
  if ((sprintf_s(devName, sizeof(devName), "%s", deviceName.c_str())) < 0) {
    CAMERA_LOGE("%{public}s: sprintf devName failed 1", __func__);
  }
...
void HosFileFormat::V4L2MatchDevice(std::vector<std::string>& cameraIDs) {
  ...
  for (size_t i = 0; i < sensorNum; ++i) {
    ...
    if (rc == RC_ERROR) {
      CAMERA_LOGE("%{public}s: V4L2GetCapability fail", __func__);
      HosV4L2Dev::v4l2Handle_->close(fd);
      continue;
    }
...
int HosFileFormat::V4L2SearchBufType(int fd) {
  ...
  if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
    CAMERA_LOGE(
        "V4L2SearchBufType capabilities is not support V4L2_CAP_STREAMING\n");
    return static_cast<int>(V4L2_BUF_TYPE_PRIVATE);
  }
