    ...
    if (itrNext == control.end() || itr->ctrl_class != itrNext->ctrl_class) {
      ...
      if (ret) {
        ...
        for (int i = 0; count > 0; i++, count--) {
          ...
          ret = HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_S_CTRL, &ctrl);
          if (ret) {
            CAMERA_LOGE(
                "HosV4L2Control::V4L2SetCtrls VIDIOC_S_CTRL error i = "
                "%{public}d\n",
                i);
            continue;
          }
...
RetCode HosV4L2Control::V4L2GetCtrl(int fd, unsigned int id, int& value) {
  ...
  rc = HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_G_CTRL, &ctrl);
  if (rc < 0) {
...
RetCode HosV4L2Control::V4L2SetCtrl(int fd, unsigned int id, int value) {
  ...
  rc = HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_S_CTRL, &ctrl);
  if (rc < 0) {
...
int HosV4L2Control::ExtControl(int fd, struct v4l2_queryctrl* ctrl) {
  ...
  ret = HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_QUERYCTRL, ctrl);
...
void HosV4L2Control::V4L2SetValue(int fd, std::vector<DeviceControl>& control,
  ...
  if (rc != RC_ERROR) {
    ctrl.value = value;
    CAMERA_LOGD(
        "%-14s : id=%08x, type=%{public}d, minimum=%{public}d, "
        "maximum=%{public}d\n"
        "\t\t value = %{public}d, step=%{public}d, default_value=%{public}d\n",
        qCtrl.name, qCtrl.id, qCtrl.type, qCtrl.minimum, qCtrl.maximum, value,
  ...
  while (!ExtControl(fd, &qCtrl)) {
    ...
    if (qCtrl.type == V4L2_CTRL_TYPE_MENU) {
      ...
      for (menu.index = qCtrl.minimum; menu.index <= qCtrl.maximum;
           menu.index++) {
        menu.id = qCtrl.id;
        rc = HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_QUERYMENU, &menu);
        if (rc < 0) {
          continue;
        }
        CAMERA_LOGD("\t %{public}d : %{public}s\n", menu.index, menu.name);
...
int HosV4L2Control::V4L2GetControl(int fd, std::vector<DeviceControl>& control,
  ...
  if (rc < 0) {
    CAMERA_LOGE(
        "V4L2GetControl HosV4L2Dev::v4l2Handle_->ioctl error rc %{public}d\n",
        rc);
    return RC_ERROR;
  }
  ...
  if (queryCtrl.type == V4L2_CTRL_TYPE_MENU) {
    ...
    for (menu.index = queryCtrl.minimum; menu.index <= queryCtrl.maximum;
         menu.index++) {
