...
RetCode HosV4L2Streams::V4L2StreamOn(int fd)
{
    ...
    CAMERA_LOGD("HosV4L2Streams::V4L2StreamOn success\n");
...
RetCode HosV4L2Streams::V4L2StreamFPSGet(int fd, DeviceFormat& format)
{
    ...
    rc = HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_G_PARM, &Stream_Parm);
    if (rc < 0) {
        CAMERA_LOGE("error: HosV4L2Dev::v4l2Handle_->ioctl VIDIOC_G_PARM failed: %{public}s\n", strerror(errno));
        return RC_ERROR;
    }
...
RetCode HosV4L2Streams::V4L2StreamFPSSet(int fd, DeviceFormat& format)
{
    ...
    rc = HosV4L2Dev::v4l2Handle_->ioctl(fd, VIDIOC_S_PARM, &Stream_Parm);
    if (rc < 0) {
        CAMERA_LOGE("error: HosV4L2Dev::v4l2Handle_->ioctl VIDIOC_S_PARM failed: %{public}s\n", strerror(errno));
        return RC_ERROR;
    }
