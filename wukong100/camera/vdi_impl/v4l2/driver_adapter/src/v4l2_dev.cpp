...
V4l2AdapterInterface* HosV4L2Dev::CreateV4l2AdapterInterface() {
  ...
  if (handler == nullptr) {
    CAMERA_LOGE(
        "failed to dlopen  /vendor/lib64/libcamv4l2adapter.z.so, %{public}s",
        dlerror());
    return nullptr;
  }
...
RetCode HosV4L2Dev::UpdateSetting(const std::string& cameraID,
  ...
  CAMERA_LOGD(
      "HosV4L2Dev::UpdateSetting: cameraID = %{public}s, command = %{public}d, "
      "args = %{public}d\n",
  ...
  switch (command) {
    ...
    case CMD_FLASH_MODE:
      ...
      switch ((int32_t)(args[0])) {
        ...
        default:
          CAMERA_LOGE(
              "HosV4L2Dev::UpdateSetting: flashmode{%{public}d} unsupported\n",
              (int32_t)(args[0]));
          break;
      }
      break;
    case CMD_FOCUS_REGION:
      ...
      CAMERA_LOGE(
          ...
          "VIDIOC_S_INPUT,l:%{public}d,t:%{public}d,w:%{public}d,h:%{public}d,"
          "\n",
